...
const RFD_RUNWAYS = ["07L", "07C", "07R", "25L", "25C", "25R"];
...
function sidsForRunway(rwy) {
  return RFD_DATA.sids.IRFD.filter((s) =>
    s.rwyProcs.some((rp) => rwysMatch(rp.rwys, rwy))
  );
}
function starsForRunway(rwy) {
  return RFD_DATA.stars.IRFD.filter((s) =>
    s.rwyProcs.some((rp) => rwysMatch(rp.rwys, rwy))
  );
}
...
function buildRoute(cfg) {
  ...
  if (adep === "IRFD" && sidIdx != null) {
    const sid = RFD_DATA.sids.IRFD[sidIdx];
  ...
  if (ades === "IRFD" && starIdx != null) {
    const star = RFD_DATA.stars.IRFD[starIdx];
...
export default function Gateway() {
  ...
  const availableSids = useMemo(() => sidsForRunway(adepRwy), [adepRwy]);
  const availableStars = useMemo(() => (ades === "IRFD" ? starsForRunway(adesRwy) : []), [ades, adesRwy]);
      ...
      <div className="gw-body">
        ...
        <div className="gw-panel gw-builder">
          ...
          <div className="gw-row2">
            ...
            <Field label="RWY">
              <Select value={adepRwy} onChange={setAdepRwy} options={RFD_RUNWAYS.map((r) => ({ value: r, label: r }))} placeholder="—" disabled={adep !== "IRFD"} />
            </Field>
          </div>
            ...
            <>
              <Field label="SID">
                <Select
                  ...
                  options={availableSids.map((s) => ({ value: RFD_DATA.sids.IRFD.indexOf(s), label: s.name }))}
                  placeholder="No SID (vectors)"
                />
              </Field>
              {sidIdx != null && RFD_DATA.sids.IRFD[sidIdx].transitions.length > 0 && (
                <Field label="SID TRANSITION">
                  <Select
                    value={sidTransIdx}
                    onChange={(v) => setSidTransIdx(v === null ? null : Number(v))}
                    options={RFD_DATA.sids.IRFD[sidIdx].transitions.map((t, i) => ({ value: i, label: t.name }))}
          ...
          <div className="gw-row2">
            ...
            <Field label="RWY">
              <Select value={adesRwy} onChange={setAdesRwy} options={RFD_RUNWAYS.map((r) => ({ value: r, label: r }))} placeholder="—" disabled={ades !== "IRFD"} />
            </Field>
          </div>
            ...
            <>
              <Field label="STAR">
                <Select
                  ...
                  options={availableStars.map((s) => ({ value: RFD_DATA.stars.IRFD.indexOf(s), label: s.name }))}
                  placeholder="No STAR (vectors)"
                />
              </Field>
              {starIdx != null && RFD_DATA.stars.IRFD[starIdx].entryTransitions.length > 0 && (
                <Field label="STAR TRANSITION">
                  <Select
                    value={starTransIdx}
                    onChange={(v) => setStarTransIdx(v === null ? null : Number(v))}
                    options={RFD_DATA.stars.IRFD[starIdx].entryTransitions.map((t, i) => ({ value: i, label: t.name }))}
                    placeholder="Select transition"
                  />
                </Field>
              )}
